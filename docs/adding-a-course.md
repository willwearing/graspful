...
Run `graspful review course.yaml` during iteration to catch structural and content issues early. See [Course Review Gate](./course-review-gate.md) for the full review specification and all 10 mechanical checks.
...
Run `graspful review course.yaml` to validate content quality. See [Course Review Gate](./course-review-gate.md) for the full content review checklist, deduplication checks, and difficulty staircase requirements.
...
```bash
graspful import course.yaml --org <slug> --publish
```

This runs the review gate inline. If all checks pass, the course goes live. If any fail, it imports as a draft with failure details.
