...
import { QuizView } from '@/lib/server-views/activity';
...
export default async function Page({ params }: { params: Promise<{ orgSlug: string; courseSlug: string }> }) {
  ...
  return <QuizView orgSlug={orgSlug} courseId={course.id} token={token} routes={learnLearnerRoutes(orgSlug, courseSlug, course.id)} />;
}
