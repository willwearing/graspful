...
import { ApiError, type ApiFetcher } from "@/lib/api";
...
export async function CourseView({ courseId, orgSlug, fetcher: apiFetch, routes }: {
  courseId: string;
  orgSlug: string;
  fetcher: ApiFetcher;
  ...
  const academy = graph.course.academyId
    ? await apiFetch<AcademyDetail>(`/orgs/${orgSlug}/academies/${graph.course.academyId}`)
    : null;
  const academyHref = academy ? routes.academy(academy.id, academy.slug) : routes.home;
  const studyEntryHref = academy ? routes.academyStudy(academy.id, academy.slug) : routes.study;
  const diagnosticHref = academy ? routes.academyDiagnostic(academy.id, academy.slug) : routes.diagnostic;
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 md:px-8">
      ...
      {courseUnlocked ? (
        ...
        <div className="rounded-lg border border-border p-6 mb-8 text-center">
          ...
          <Button render={<Link href={diagnosticHref} />}>
            Take Diagnostic
          </Button>
        </div>
      )}
          ...
          <div className="grid gap-4 mb-8">
            {sectionProgress
              ...
              .map((item) => {
                ...
                const sectionHref = getSectionHref({
                  courseId,
                  courseUnlocked,
                ...
                const href = sectionHref ? routes.fromCoursePath(sectionHref) : null;
